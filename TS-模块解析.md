...
> 这节假设你已经了解了模块的一些基本知识 请阅读[模块](https://typescript.bootcss.com/Modules.md)文档了解更多信息。
...
如果它们失败了并且如果模块名是非相对的（且是在`"moduleA"`的情况下），编译器会尝试定位一个[外部模块声明](https://typescript.bootcss.com/Modules.md#ambient-modules)。 我们接下来会讲到非相对导入。
...
共有两种可用的模块解析策略：[Node](https://typescript.bootcss.com/module-resolution.html#node)和[Classic](https://typescript.bootcss.com/module-resolution.html#classic)。 你可以使用`--moduleResolution`标记指定使用哪种模块解析策略。 若未指定，那么在使用了`--module AMD | System | ES2015`时的默认值为[Classic](https://typescript.bootcss.com/module-resolution.html#classic)，其它情况时则为[Node](https://typescript.bootcss.com/module-resolution.html#node)。
...
1. `"*"`： 表示名字不发生改变，所以映射为`` => `/`
2. `"generated/*"`表示模块名添加了“generated”前缀，所以映射为`` => `/generated/`
